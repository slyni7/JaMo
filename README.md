...
`ㅁ`은 조건, `ㅃ`은 반복, `ㅂ`은 출력. `+ - * /`는 그대로 두고, 새 이름은 `ㅒ`로 소개합니다. 짧게 적는 자모 문법을 중심으로 영문·한글 별칭을 직접 선택할 수 있습니다.
...
> 자모·모아쓰기 문법과 `ㅊ`의 새 명령·자료형 정의를 구현했고, 브라우저와 Node CLI에서 검증합니다. 설정에서 겹자모 입력 보조, 스킨, 소리, 사용할 자모와 대응단어를 편집합니다. *이탤릭체 별칭 목록*은 선택할 수 있는 후보이며 기본 예약어 목록이 아닙니다. GitHub Pages에 공개했고 기본 실행·모아쓰기·오류 표시·오류 후 재실행을 확인했습니다. 실제 휴대전화 입력은 아직 시험하지 않았습니다.
...
짧은 자모와 익숙한 단어를 같은 내부 명령으로 연결합니다. 설정에서 사용할 대응단어를 직접 입력하고, 기본 영문·한글 후보를 넣거나 비울 수 있습니다. 초기 별칭은 비어 있으므로 선택하지 않은 후보가 자동으로 예약어가 되지 않습니다.
...
`ㅂ`의 대응단어에 `print, 보여줘, 인쇄, 프린트`를 넣으면 모두 같은 출력을 뜻합니다. 문자열이나 긴 이름의 일부는 바꾸지 않습니다. 서로 중복된 별칭은 설정 적용 전에 오류로 표시합니다. 대응단어가 선언보다 우선하므로 기존 이름과 충돌하면 그 이름도 명령으로 읽으며, 소스의 이름 충돌을 미리 나열하는 화면과 자동 표기 변환은 아직 없습니다. 호스트 코드는 `Interpreter`의 `wordAliases` 맵으로 같은 대응단어를 등록할 수 있습니다.

**입력 경계는 표기에 따라 다릅니다. 영문 `if` 뒤에는 공백을 요구하고, 자모 `ㅁ` 뒤에는 공백을 요구하지 않습니다.** `ㅁ_조건`, `ㅁ(조건)`, `ㅁㅖ`처럼 붙여 쓸 수 있습니다. `_조건`의 밑줄은 변수 이름의 일부이며 제거하지 않습니다. 설정에서 `if`를 등록하거나 `ㅊif = ㅁ`으로 직접 정의하면 `if 조건`을 허용하고 `if(조건)`은 거부합니다.
...
별칭은 같은 연산을 다른 이름으로 적는 선택지입니다. 예를 들어 *tuple*을 등록하더라도 별도의 불변 튜플 자료형이 생기는 것은 아닙니다. 표의 후보 전체를 자동으로 등록하지 않으며, 선택한 별칭만 선언된 이름보다 먼저 적용합니다.
...
톱니바퀴에서 다음 항목을 설정합니다. **40개 명령의 기능과 11개 겹자모의 분해, `ㅛ` 압축 규칙은 고정**이며, 기능 재배정은 하지 않습니다.

| 항목 | 현재 동작 |
| --- | --- |
| 겹자모 입력 | 기본 켜짐·300ms. 연속 자음 11쌍을 결합하며 대기시간을 0~2000ms로 조절합니다. 첫 글자는 즉시 표시하고, 설정 시간 이내의 다음 글자로 결합합니다. |
| 스킨 | 종이·밤·고대비와 강조색을 선택합니다. 취소하면 적용 전 색으로 돌아갑니다. |
| 소리 | 기본 꺼짐·음량 20%. 입력·실행 완료·오류·입력/점검 대기에 소리를 내며, 음량과 미리 듣기를 제공합니다. |
| 사용할 자모 | 51개 각각 켜기·끄기, 전체 51개·핵심 18개·기본 40개·모두 끄기 구성을 제공합니다. |
| 대응단어 | 51개 표기에 연결할 한글·영문 별칭을 쉼표로 편집합니다. 기본 명령별 영문·한글 후보 넣기와 전체 비우기를 제공합니다. |

입력 보조는 소스 편집기와 화면 자모 키보드에 적용합니다. 붙여넣기·선택 영역 덮어쓰기·입력기 조합 중에는 글자를 합치지 않으며, 커서 이동·삭제·포커스 이탈·파일/설정 변경은 대기를 취소합니다. 이미 입력된 소스 전체를 다시 결합하지 않습니다.

꺼진 자모가 명령으로 쓰이면 **실행 전 구문 분석에서 거부**합니다. 대응단어·매크로·모아쓰기·가져온 모듈에도 적용하므로 실행되지 않는 분기에 적힌 명령도 검사합니다. 선언한 이름·문자열·주석은 유지합니다. `ㅄ`을 끄면 그 표기가 막히고, 별도로 입력한 `ㅂㅅ`은 두 기본 자모의 설정을 따릅니다. `ㅂ`이나 `ㅅ`이 꺼졌다면 `ㅄ`로 우회할 수 없습니다. `ㅛ`를 끄면 `묙` 같은 압축 표기도 사용할 수 없습니다.

설정은 적용 후 **다음 실행부터** 사용합니다. 현재 설정 저장은 연결하지 않아 새로고침하면 기본값으로 돌아갑니다. 기존 열린 소스 파일의 자동 저장은 유지합니다. 후속 저장 방식으로는 설정을 서버에 보내지 않고 브라우저에 보관하는 `localStorage`를 검토합니다. 쿠키는 요청마다 전송되며 보통 개당 4KB로 제한되므로 이 용도에는 불필요합니다. [MDN의 클라이언트 저장소 비교](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Cookies), [localStorage의 유지 범위](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage)
...
| TypeScript 프런트엔드·런타임 | 중첩 매크로 종료 수정 판의 타입 검사·빌드와 자동 시험 262/262개를 통과했습니다. 기존 Python과의 차등 검증 93/93개는 공백 규칙 정정 전 판의 기록입니다. |
| `ㅊ`와 조건문의 경계 | `ㅊ`의 새 기능·자료형 역할은 사용자 요구이며 세부 문법은 구현 선택입니다. 공백은 영문 `if`에 요구하고 자모 `ㅁ`에는 요구하지 않는다는 사용자 정정을 반영했습니다. |
| 영문·한글 별칭 | 설정 화면과 호스트의 별칭 등록을 지원합니다. 이탤릭체 항목은 후보이며 기본 예약어로 전부 등록하지 않습니다. |
...
자동 시험 262개는 프런트엔드 73, 런타임 53, 모아쓰기·우선순위·인코딩 24, `ㅛ` 압축 문법 37, 실제 CLI 8, README·기본 예제 15, 입력 보조 22, 설정·실행 제한 18, 사운드 7, 브라우저 Worker 5개로 구성됩니다. Node의 집계 기준으로 최상위 시험 256개와 사운드 하위 시험 6개를 포함합니다. [현재 시험 출력](verification/macro-close-tests.tap)과 [이전 모아쓰기 웹 실행 기록](verification/hangul-browser.json)을 제공합니다. 겹자모 11개는 대응하는 현대 종성과의 분해 일치, UTF-8·CP949 왕복, 선언·custom 이름 보존을 확인했습니다. 현대 완성형 11,172개 모두의 분해를 JavaScript의 표준 NFD 결과와 대조했고 CP949 저장·읽기도 같은 범위를 왕복 확인했습니다. 이는 모든 음절 조합이 유효한 프로그램이라는 뜻은 아닙니다.

중첩 매크로에서 `ㅋ ㅋ`를 내놓는 별칭을 다시 펼쳐 종료 기호가 중복되는 문제를 수정했습니다. 닫는 데 사용한 부분만 보관하고 일반 본문·소개 별칭은 호출 시점의 정의를 따릅니다. 종료 전후의 실행 토큰, 2~5개 블록의 연속 종료, 재정의, 100회 재사용, `ㅛ` 전달 본문, 부족·초과 종료와 순환 오류를 여섯 회귀 시험에서 확인합니다. 새 예약어나 자모 기능 변경은 없습니다. 별칭으로 도입한 선언, 브라우저 입력 되돌리기, 진입 파일의 순환 import 검사, Enter 파일 생성·오류 위치 이동의 앞선 수정도 유지합니다.

압축 문법 시험에는 초성 19개 × 종성 27개인 513개 표기를 포함합니다. 고정 내용 `ㅖ`와 세 가지 문맥에서 풀어 쓴 기준 소스 1,539건을 만들고, 완성형·NFD 두 표기와 3,078회 비교했습니다. 유효한 기준 구문 4건은 AST를 비교하고, 나머지 1,535건은 모두 구문 오류인지 확인했습니다. 실제 실행 시험은 조건·반복·함수·임의 토큰 조합·매크로 중첩·이름 우선순위·라벨 이동을 별도로 다룹니다. [로컬 웹 실행 기록](verification/yo-combo-browser.json)은 Worker 실행 4/4 경로를 기록하며, 실제 Android·iPhone 입력기 검증은 포함하지 않습니다.

[설정 화면 검증 기록](verification/settings-browser.json)은 실제 내장 브라우저에서 확인한 10개 경로를 기록합니다. 별칭 실행·꺼진 명령 거부·압축 유지·중복 거부·설정 취소·51개 표시·프리셋·283ms 간격 결합·320/390px 화면·소리 설정 조작을 확인했습니다. Web Audio는 가짜 오디오 장치를 사용한 제어 시험을 포함하며, 실제 스피커 청취와 Android/iPhone 물리 키보드·입력기는 검증하지 않았습니다.
