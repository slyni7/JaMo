...
export const CLUSTER_DEFAULTS: Readonly<Record<string, string>> = Object.freeze({
  'ㄳ':'ㄱㅅ','ㄵ':'ㄴㅈ','ㄶ':'ㄴㅎ','ㄺ':'ㄹㄱ','ㄻ':'ㄹㅁ','ㄼ':'ㄹㅂ',
  'ㄽ':'ㄹㅅ','ㄾ':'ㄹㅌ','ㄿ':'ㄹㅍ','ㅀ':'ㄹㅎ','ㅄ':'ㅂㅅ',
});
export const SYMBOLS: readonly string[] = [...COMMANDS.map(([symbol]) => symbol), ...Object.keys(CLUSTER_DEFAULTS)];
export const BASIC_SYMBOLS = Array.from('ㅒㅖㅗㅜㅂㅅㅆㅉㅌㅐㅁㄱㅇㅋㄴㅘㅣㅔ');
export type Theme = 'paper' | 'night' | 'contrast';
export interface Settings {
  version: 1;
  assist: { enabled: boolean; timeoutMs: number };
  theme: Theme;
  accent: string;
...
export function copySettings(settings: Settings): Settings { return structuredClone(settings); }
export function validateSettings(input: unknown): Settings {
  if (!input || typeof input !== 'object') throw new Error('설정 형식이 올바르지 않습니다.');
  const candidate = input as Settings;
  if (candidate.version !== 1) throw new Error('지원하지 않는 설정 버전입니다.');
  ...
  if (!candidate.sound || typeof candidate.sound.enabled !== 'boolean' || !Number.isFinite(candidate.sound.volume)
    || candidate.sound.volume < 0 || candidate.sound.volume > 1) throw new Error('음량은 0~100%로 설정하세요.');
  ...
  for (const symbol of SYMBOLS) {
    ...
    if (!Array.isArray(aliases) || aliases.length > 50) throw new Error(`${symbol}의 별칭은 50개 이하로 입력하세요.`);
    for (const alias of aliases) {
      ...
      if (used.has(alias)) throw new Error(`별칭 '${alias}'이 ${used.get(alias)}와 ${symbol}에 중복되어 있습니다.`);
      used.set(alias, symbol);
    }
  }
  const clean = copySettings(defaultSettings());
  clean.assist = { ...candidate.assist }; clean.theme = candidate.theme; clean.accent = candidate.accent;
  clean.sound = { ...candidate.sound }; clean.disabledSymbols = [...candidate.disabledSymbols];
  for (const symbol of SYMBOLS) {
    clean.aliases[symbol] = [...candidate.aliases[symbol]];
  }
  return clean;
}
export function wordAliases(settings: Settings): [string, string][] {
  return SYMBOLS.flatMap(symbol => settings.aliases[symbol].map(alias => [alias, symbol] as [string, string]));
}
