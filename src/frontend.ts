...
export class ParseError extends Error {
  ...
  override toString(): string { return `${this.line}행 ${this.col}열: ${this.message}`; }
}
...
const LITERALS = new Map<string, boolean | null>([["ㅖ", true], ["ㅗ", false], ["ㅜ", null]]);
export const MAX_SYNTAX_DEPTH = 128;

function jamo(char: string): boolean {
...
const EXTRA_DIGITS: readonly (readonly [number, number])[] = [
...
function nameStart(char: string): boolean {
  return char === "_" || (/^\p{L}$/u.test(char) && !jamo(char));
}
function namePart(char: string): boolean {
  if (jamo(char)) return false;
  if (nameStart(char) || /^[\p{Nd}\p{Mn}\p{Mc}]$/u.test(char)) return true;
  const point = char.codePointAt(0)!;
...
const digit = (char: string | undefined): boolean => char !== undefined && /^[0-9]$/.test(char);
const whitespace = (char: string | undefined): boolean => char !== undefined && /^[ \t\f\v\n\r]$/.test(char);
...
class Lexer {
  ...
  col = 1;
  tokens: Token[] = [];
  delimiters: { char: string; line: number; col: number }[] = [];
  ...
  emit(kind: string, value: any, line = this.line, col = this.col): void {
    this.tokens.push({ kind, value, line, col, whitespaceAfter: whitespace(this.chars[this.pos]) });
  }
  pair(): string { return this.chars.slice(this.pos, this.pos + 2).join(""); }
  newline(): void {
  ...
  string(): void {
    ...
    while (this.pos < this.chars.length) {
      ...
      if (Object.hasOwn(escapes, escape)) chars.push(escapes[escape]);
      else if (["x", "u", "U"].includes(escape)) {
        const size = ({ x: 2, u: 4, U: 8 } as Record<string, number>)[escape];
        ...
        for (let index = 0; index < size; index++) this.advance();
      ...
      } else throw new ParseError(`알 수 없는 이스케이프 \\${escape}`, escapeLine, escapeCol);
    }
    throw new ParseError("문자열을 닫는 따옴표가 없습니다", line, col);
  }
  number(): void {
    const begin = this.pos, { line, col } = this;
    while (digit(this.chars[this.pos])) this.advance();
    let floating = false;
    ...
    if (["e", "E"].includes(this.chars[this.pos])) {
      floating = true;
      this.advance();
      if (["+", "-"].includes(this.chars[this.pos])) this.advance();
      const beginExponent = this.pos;
      while (digit(this.chars[this.pos])) this.advance();
      if (this.pos === beginExponent) throw new ParseError("지수 뒤에 숫자가 필요합니다", line, col);
    }
  ...
  run(): Token[] {
    while (this.pos < this.chars.length) {
      const char = this.chars[this.pos];
      if ([" ", "\t", "\f", "\v"].includes(char) || (char === "\ufeff" && this.pos === 0)) this.advance();
      else if (char === "\n") this.newline();
      else if (char === "#") {
        ...
        while (this.pos < this.chars.length && this.pair() !== "*/") {
          if (this.chars[this.pos] === "\n") this.newline(); else this.advance();
        }
        if (this.pos >= this.chars.length) throw new ParseError("여러 줄 주석을 닫는 */가 없습니다", line, col);
        this.advance(); this.advance();
      } else if (["'", '"'].includes(char)) this.string();
      else if (digit(char) || (char === "." && digit(this.chars[this.pos + 1]))) this.number();
        ...
        while (this.pos < this.chars.length && jamo(this.chars[this.pos])
          && !KEYWORDS.has(this.chars[this.pos]) && commandParts(this.chars[this.pos]) !== null) this.advance();
        this.emit("NAME", this.chars.slice(begin, this.pos).join(""), line, col);
      } else if (jamo(char)) throw new ParseError(`자모 '${char}'에는 배정된 문법이 없습니다`, this.line, this.col);
      else if (nameStart(char)) {
        const begin = this.pos, { line, col } = this;
        this.advance();
        while (this.pos < this.chars.length && namePart(this.chars[this.pos])) this.advance();
        this.emit("NAME", this.chars.slice(begin, this.pos).join(""), line, col);
      } else if (char === ";") { this.emit("SEP", ";"); this.advance(); }
      else {
        const { line, col } = this;
        const pair = this.pair();
        if (["==", "!=", "<=", ">=", "//", "**", "=>"].includes(pair)) {
          this.emit(pair, pair); this.advance(); this.advance();
        } else if ("+-*/%<>=()[]:,.".includes(char)) {
          if ("([".includes(char)) this.delimiters.push({ char, line, col });
          else if (")]".includes(char)) {
            const expected = char === ")" ? "(" : "[";
          ...
          this.emit(char, char); this.advance();
        } else throw new ParseError(`알 수 없는 문자 ${JSON.stringify(char)}`, line, col);
      }
    }
...
function spellingTokens(token: Token): Token[] {
...
function packCombos(tokens: Token[]): Token[] {
  const adjacent = (left: Token, right?: Token) => right && left.line === right.line
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    if (!CONSONANTS.has(first.kind) || middle?.kind !== "ㅛ" || !CONSONANTS.has(last?.kind)
      || !adjacent(first, middle) || !adjacent(middle, last)) { result.push(first); continue; }
    const suffix = [last];
...
function resolveSpellings(raw: Token[], options: ParseOptions): Token[] {
  ...
  const commandAt = (index: number): { token: Token; next: number } => {
    ...
    while (next < tokens.length && next < index + 3) {
      ...
      if (next > index && candidate.kind === "NAME" && !decomposableSpelling(candidate.value)) break;
      appendTokens(probe, candidate.kind === "NAME" && names.has(candidate.value) ? [candidate] : spellingTokens(candidate)); next++;
  ...
  const introduce = (index: number, register = true) => {
    if (tokens[index]?.kind !== "NAME") return;
  ...
  const collect = (combos: boolean) => { for (let index = 0; index < tokens.length; index++) {
    ...
    while (effective.kind === "NAME" && macroTails.has(effective.value) && !visited.has(effective.value)
      ...
      visited.add(effective.value); effective = macroTails.get(effective.value)!;
    }
...
function appendTokens(target: Token[], source: Iterable<Token>): void {
  for (const token of source) target.push(token);
}

/** The wrapper supplies a token payload; it does not add expression parentheses. */
function expandCombo(tokens: Token[], start: number): { body: Token[]; end: number } {
  ...
  for (; end < tokens.length; end++) {
    ...
    if (nesting === 0 && (wrapped ? part.kind === ")" : ["SEP", "SOFT_NL", "EOF", ")", "]"].includes(part.kind))) break;
    ...
    if (part.kind === "SOFT_NL") {
      ...
      if (wrapped && nesting === 0 && token.value.suffix.some((item: Token) => item.kind === "ㅋ"))
        payload.push({ ...part, kind: "SEP" });
      continue;
    }
    if (["(", "["].includes(part.kind)) nesting++;
    else if ([")", "]"].includes(part.kind)) nesting--;
  ...
  const origin = (part: Token): Token => ({ ...part, line: token.line, col: token.col,
...
function spliceTokens(tokens: Token[], start: number, count: number, body: Token[]): void {
...
class MacroProcessor {
  private produced = 0;
  private readonly macros: Macros = new Map();
  ...
  private inlineEnd(tokens: Token[], start: number): number {
    let end = start;
    while (end < tokens.length && !["SEP", "EOF"].includes(tokens[end].kind)) end++;
    return end;
  }

  private typeHeaderEnd(tokens: Token[], start: number): number {
    let depth = 0;
    for (let index = start + 2; index < tokens.length; index++) {
      if (tokens[index].kind === "(") depth++;
      else if (tokens[index].kind === ")" && --depth === 0) return index + 1;
    }
    const token = tokens[start];
    throw new ParseError("custom 타입의 매개변수 괄호가 닫히지 않았습니다", token.line, token.col);
  }

  private nameAt(tokens: Token[], start: number): string {
    const token = tokens[start + 1] ?? tokens[start];
    if (token.kind !== "NAME") throw new ParseError("ㅊ 뒤에는 등록할 이름이 필요합니다", token.line, token.col);
    return token.value;
  }

  /** Expand known names only for delimiter accounting; keep body references lexical. */
  private balanceTokens(tokens: Token[], definitions: Macros, chain: string[] = []): Token[] {
    const result: Token[] = [];
    for (let index = 0; index < tokens.length; index++) {
      ...
      if (token.kind === "ㅊ") {
        ...
        result.push(token);
        if (tokens[index + 1]) result.push(tokens[++index]);
    ...
    for (let index = start + 3; index < tokens.length; index++) {
      ...
      for (let part = 0; part < expanded.length; part++) {
        ...
        if (virtual.kind === "NAME" && local.has(virtual.value)) {
          ...
          const replacement = this.balanceTokens(this.replacement(virtual, local.get(virtual.value)!, chain),
            local, [...chain, virtual.value]);
  ...
  private expand(tokens: Token[]): Token[] {
    const result: Token[] = [];
    for (let index = 0; index < tokens.length; index++) {
      const token = tokens[index];
      if (token.kind === "COMBO") {
...
class Parser {
  static readonly BINDING: Record<string, number> = {
    "ㅞ": 5, "ㅣ": 10, "ㅘ": 20, "==": 30, "!=": 30, "<": 30, "<=": 30,
    ">": 30, ">=": 30, "ㅔ": 30, "+": 40, "-": 40, "*": 50, "/": 50,
    "//": 50, "%": 50, "**": 70,
  };
  static readonly COMPARISONS = new Set(["==", "!=", "<", "<=", ">", ">=", "ㅔ"]);
  ...
  get current(): Token { return this.tokens[this.pos]; }
  take(): Token { return this.tokens[this.pos++]; }
  accept(kind: string): Token | null { return this.current.kind === kind ? this.take() : null; }
  expect(kind: string, message?: string): Token {
    if (this.current.kind !== kind)
      throw new ParseError(message ?? `${kind}이(가) 필요합니다`, this.current.line, this.current.col);
    return this.take();
  }
  name(): string { return this.expect("NAME", "이름이 필요합니다. 자모 키워드는 이름으로 쓸 수 없습니다").value; }
  node(kind: string, token: Pick<Token, "line" | "col">, args: Record<string, any> = {}): Node {
    return { kind, args, line: token.line, col: token.col };
  }
  skipSeparators(): void { while (this.accept("SEP")) { /* physical statement separation */ } }
  headerEnd(): void {
  ...
  block(stoppers: Set<string>): Node[] {
    this.enterSyntax();
    try {
      const result: Node[] = [];
      this.skipSeparators();
  ...
  checkTreeDepth(nodes: Node[]): void {
    const pending: [Node, number][] = nodes.map(node => [node, 1]);
    while (pending.length) {
      ...
      while (values.length) {
        ...
        if (Array.isArray(value)) for (const item of value) values.push(item);
          ...
          pending.push([value as Node, depth + 1]);
      }
    }
  }
  statement(): Node {
    const token = this.current, kind = token.kind;
    if (kind === "ㅊ") return this.customType();
    if (kind === "ㅒ") {
      this.take(); const name = this.name();
      this.expect("=", "새 선언에는 =와 초기값이 필요합니다");
      return this.node("declare", token, { name, value: this.expression() });
    }
    if (kind === "ㅁ") return this.ifStatement();
    if (["ㄸ", "ㅃ"].includes(kind)) {
      this.take();
      let name: string | null = null, iterable: Node | null = null, condition: Node | null = null;
      if (kind === "ㅃ") {
        name = this.name(); this.expect("ㅔ", "for의 이름 뒤에는 ㅔ가 필요합니다"); iterable = this.expression();
      } else condition = this.expression();
      this.accept("ㄱ"); this.headerEnd();
    ...
    if (kind === "ㅎ") {
      ...
      const params: string[] = [];
      if (this.current.kind !== ")") while (true) {
        ...
        if (!this.accept(",") || this.current.kind === ")") break;
      }
      this.expect(")"); this.headerEnd();
      const body = this.block(new Set(["ㅋ"])); this.expect("ㅋ", "함수 블록을 닫는 ㅋ가 없습니다");
      return this.node("function", token, { name, params, body });
    }
    if (["ㄷ", "ㅠ", "ㅙ"].includes(kind)) {
      this.take(); const value = ["SEP", "EOF"].includes(this.current.kind) ? null : this.expression();
      return this.node(({ "ㄷ": "return", "ㅠ": "throw", "ㅙ": "debug" } as Record<string, string>)[kind], token, { value });
    }
    if (["ㅡ", "ㄹ", "ㅍ"].includes(kind)) {
      this.take();
      return this.node(({ "ㅡ": "break", "ㄹ": "continue", "ㅍ": "pass" } as Record<string, string>)[kind], token);
    }
    if (kind === "ㅑ") {
      this.take(); this.headerEnd();
      ...
      this.headerEnd(); const handler = this.block(new Set(["ㅋ"]));
  ...
  requireIfWhitespace(token: Token): void {
  ...
  ifStatement(): Node {
    const token = this.expect("ㅁ"); this.requireIfWhitespace(token);
    const branches: [Node, Node[]][] = [];
    let otherwise: Node[] | null = null;
    while (true) {
      const condition = this.expression();
      this.expect("ㄱ", "if 조건 뒤에는 ㄱ이 필요합니다"); this.headerEnd();
      branches.push([condition, this.block(new Set(["ㅇ", "ㅋ"]))]);
      if (!this.accept("ㅇ")) break;
      const elseif = this.accept("ㅁ");
      if (elseif) { this.requireIfWhitespace(elseif); continue; }
      this.headerEnd(); otherwise = this.block(new Set(["ㅋ"])); break;
    }
    this.expect("ㅋ", "조건 블록을 닫는 ㅋ가 없습니다");
    return this.node("if", token, { branches, otherwise });
  }
  expression(minimum = 0): Node {
    this.enterSyntax();
    try { return this.expressionInner(minimum); } finally { this.nesting--; }
  }
  expressionInner(minimum: number): Node {
    let token = this.take(); const initialKind = token.kind;
    let left: Node;
    if (["NUMBER", "STRING", "CHAR"].includes(initialKind)) {
      left = this.node("literal", token, { value: token.value });
      if (initialKind === "CHAR") left.args.char = true;
    } else if (LITERALS.has(initialKind)) left = this.node("literal", token, { value: LITERALS.get(initialKind) });
    ...
    else if (initialKind === "(") { left = this.expression(); this.expect(")", "식을 닫는 )가 필요합니다"); }
    else if (initialKind === "[") left = this.node("list", token, { items: this.commaExpressions("]") });
    else throw new ParseError("값이나 이름으로 시작하는 식이 필요합니다", token.line, token.col);
    let sawComparison = false;
    while (true) {
      token = this.current; const kind = token.kind;
      if (kind === "(" && 90 >= minimum) {
        this.take(); left = this.node("call", left, { callee: left, args: this.commaExpressions(")") }); continue;
      }
      if (kind === "[" && 90 >= minimum) {
        this.take(); const start = this.current.kind === ":" ? null : this.expression();
        if (this.accept(":")) {
          const stop = [":", "]"].includes(this.current.kind) ? null : this.expression();
          let step: Node | null = null;
          if (this.accept(":")) step = this.current.kind === "]" ? null : this.expression();
      ...
      if (kind === "." && 90 >= minimum) {
        this.take(); left = this.node("member", left, { target: left, name: this.name() }); continue;
      }
      const binding = Parser.BINDING[kind] ?? -1;
      if (binding < minimum) break;
      if (Parser.COMPARISONS.has(kind)) {
        if (sawComparison) throw new ParseError("연속 비교는 괄호 또는 ㅘ로 나누어 써 주세요", token.line, token.col);
        sawComparison = true;
      }
      this.take(); const right = this.expression(kind === "**" ? 60 : binding + 1);
      left = this.node("binary", left, { op: kind, left, right });
    }
    return left;
  }
  commaExpressions(closing: string): Node[] {
    const values: Node[] = [];
    if (this.current.kind !== closing) while (true) {
      values.push(this.expression());
      if (!this.accept(",") || this.current.kind === closing) break;
    }
...
export function parse(source: string, options: ParseOptions = {}): Node[] {
  if (typeof source !== "string") throw new TypeError("source must be str");
  const parser = new Parser(source, options);
  try { return parser.run(); }
  catch (error) {
