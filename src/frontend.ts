...
class Lexer {
  ...
  newline(): void {
    if (this.delimiters.length === 0) this.emit("SEP", "\n");
    this.advance();
  }
  string(): void {
    const { line, col } = this;
...
function resolveSpellings(raw: Token[], options: ParseOptions): Token[] {
  ...
  for (let index = 0; index < tokens.length; index++) {
    if (tokens[index].kind === "ㅊ" && tokens[index + 1]?.kind === "NAME" && tokens[index + 2]?.kind === "=") {
      ...
      if (end > index + 3) macroTails.set(tokens[index + 1].value, tokens[end - 1]);
    ...
    if (["ㅒ", "ㅎ", "ㅊ", "ㅃ", "ㅏ", "ㅛ", "ㄲ"].includes(kind)) introduce(index + 1);
...
class MacroProcessor {
  ...
  private expand(tokens: Token[], chain: string[] = []): Token[] {
    const result: Token[] = [];
    for (let index = 0; index < tokens.length; index++) {
      const token = tokens[index];
      if (token.kind === "ㅊ") {
...
class Parser {
  ...
  statement(): Node {
    ...
    if (["ㄲ", "ㅛ"].includes(kind)) {
      this.take(); return this.node(kind === "ㄲ" ? "goto" : "label", token, { name: this.name() });
    }
    if (kind === "ㅚ") {
