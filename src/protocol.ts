...
export type Reply =
  ...
  | { type: 'input'; id: number; prompt: string }
  | { type: 'debug'; id: number; path: string; line: number; variables: [string, string][] }
  | { type: 'file'; path: string; text: string }
  | { type: 'done'; elapsed: number }
  | { type: 'error'; message: string; line?: number; col?: number; path?: string };
