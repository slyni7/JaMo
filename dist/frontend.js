...
function resolveSpellings(raw, options) {
    ...
    const declarationPositions = (stream, index, kind, next, record) => {
        if (["ㅒ", "ㅎ", "ㅊ", "ㅃ", "ㅏ", "ㅛ", "ㄲ"].includes(kind))
            record(next);
        if (kind === ".")
            record(index + 1, false);
        if (stream[index + 1]?.kind === "=>")
            record(index);
        ...
        for (let index = 0; index < tokens.length; index++) {
            if (tokens[index].kind === "ㅊ" && tokens[index + 1]?.kind === "NAME" && tokens[index + 2]?.kind === "=") {
                ...
                if (body.length) {
                    ...
                    const alternatives = possibleTails.get(name) ?? new Map();
    ...
    for (const name of possibleTails.keys()) {
        const kinds = new Set(), visited = new Set();
        const pending = [[name, 0]];
        for (let cursor = 0; cursor < pending.length; cursor++) {
            const [spelling, depth] = pending[cursor];
            if (visited.has(spelling) || depth >= MAX_SYNTAX_DEPTH)
                continue;
            visited.add(spelling);
            for (const tail of possibleTails.get(spelling)?.values() ?? []) {
                if (tail.kind === "NAME")
                    pending.push([tail.value, depth + 1]);
                else
                    kinds.add(tail.kind);
            }
        }
    ...
    if (candidates.size) {
        ...
        try {
            ...
            expanded.forEach((token, index) => {
                let next = index + 1;
                if (token.kind === "ㅛ" && expanded[next]?.kind === "(")
                    next++;
                while (expanded[next]?.kind === "SOFT_NL")
                    next++;
                declarationPositions(expanded, index, token.kind, next, (position, register = true) => {
                    const declared = expanded[position];
                    if (register && declared?.kind === "NAME" && candidates.has(declared.value))
                        discovered.add(declared.value);
                });
            });
...
class MacroProcessor {
    ...
    bodyTokens(tokens, start, end, completeTail = false) {
        const result = [];
    ...
    collectBlock(tokens, start, definitions, nesting = 0, boundaries = []) {
            ...
            for (let part = 0; part < expanded.length; part++) {
                ...
                if (virtual.kind === "ㅊ") {
                    ...
                    if (following[part + 2]?.kind === "SEP") {
                        ...
                        const nested = this.collectBlock(following, part, local, nesting + 1, [boundary, ...callerBoundaries.map(item => item.shifted)]);
                        local.set(nestedName, nested.body);
                        ...
                        part = Math.min(nested.end, boundary.index) - 1;
                        previous = "ㅋ";
                        continue;
                    }
                }
                if ((this.openers.has(virtual.kind) && !(virtual.kind === "ㅁ" && previous === "ㅇ")) || virtual.kind === "ㅊ")
                    ...
                    if (level === 0) {
                        ...
                        if (token.kind !== "ㅋ" || consumed !== index + 1 || part !== 0 || tail.length) {
                            ...
                            for (let cursor = index + 1; cursor < consumed; cursor++)
                                tokens[cursor].boundaryExpansion = [];
                        }
                        if (tail.length)
                            tokens[consumed - 1].boundaryTail = tail;
                        for (const boundary of boundaries)
                            if (consumed < boundary.index)
                                boundary.index += tail.length;
    ...
    run(tokens) { return this.expand(tokens.map(token => ({ ...token }))); }
}
class Parser {
    static BINDING = {
