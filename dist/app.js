...
const select = element('examples');
const inputForm = element('input-form');
const programInput = element('program-input');
const debugPanel = element('debug-panel');
const storageKey = 'jamo-workspace-v1';
let files = { ...examples[0].files };
...
function persist() {
    ...
    try {
        ...
        element('storage-note').textContent = '브라우저 저장 공간을 사용할 수 없습니다. 내려받기로 파일을 보관하세요.';
    }
}
...
function line(text, className = 'output-line') {
...
function goToError(path, lineNumber) {
...
function run() {
    ...
    try {
        ...
        showError({ type: 'error', message: `실행 환경을 열지 못했습니다: ${String(error)}` });
        return;
    }
        ...
        switch (data.type) {
            case 'output':
                if (++outputCount <= 3000)
                    ...
                    outputLimited = true;
                    line('출력이 3,000줄을 넘어 화면 표시를 생략합니다. 프로그램은 계속 실행합니다.', 'output-note');
                }
                break;
            case 'input':
...
examples.forEach((example, index) => {
    const option = new Option(example.title, String(index));
    select.add(option);
});
select.selectedIndex = -1;
const placeholder = new Option('예제 선택', '', true, true);
...
select.onchange = () => {
    ...
    let suffix = 1;
    while (Object.keys(example.files).some(path => Object.hasOwn(files, `/예제${suffix}${path}`)))
        suffix++;
    for (const [path, source] of Object.entries(example.files))
        files[`/예제${suffix}${path}`] = source;
...
stopButton.onclick = () => { if (!busy)
    return; line('실행을 중단했습니다.', 'output-note'); finish('중단됨'); };
element('clear-output').onclick = () => output.replaceChildren();
inputForm.onsubmit = event => {
    event.preventDefault();
    if (pendingInput === null)
        return;
    line(`› ${programInput.value}`, 'output-note');
    post({ type: 'input', id: pendingInput, value: programInput.value });
    pendingInput = null;
    inputForm.hidden = true;
    status('실행 중');
};
element('input-eof').onclick = () => {
    if (pendingInput === null)
        return;
    post({ type: 'input', id: pendingInput, value: '', closed: true });
    pendingInput = null;
    inputForm.hidden = true;
    status('실행 중');
};
element('debug-continue').onclick = () => {
    if (pendingDebug === null)
        return;
    post({ type: 'debug', id: pendingDebug, action: 'continue' });
    pendingDebug = null;
    debugPanel.hidden = true;
    status('실행 중');
};
...
element('download').onclick = () => {
    ...
    a.href = url;
    a.download = active.split('/').at(-1) || '프로그램.txt';
    a.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
};
const picker = element('file-picker');
...
picker.onchange = async () => {
    ...
    try {
        ...
        if (selected.some(file => Object.hasOwn(files, `/${file.name}`))) {
            let number = 2;
            while (Object.keys(files).some(path => path.startsWith(`/불러옴${number}/`)))
                number++;
            prefix = `/불러옴${number}`;
        }
        ...
        line('UTF-8 파일로 저장한 뒤 다시 열어주세요. 열린 파일은 그대로 유지했습니다.', 'output-error');
    }
    picker.value = '';
};
const dialog = element('new-file-dialog');
element('new').onclick = () => { element('new-file-error').textContent = ''; dialog.showModal(); element('new-file-name').select(); };
element('cancel-new-file').onclick = () => dialog.close('cancel');
element('new-file-form').onsubmit = event => {
    event.preventDefault();
    const raw = element('new-file-name').value.trim().replaceAll('\\', '/');
    const parts = raw.split('/').filter(Boolean);
    const path = '/' + parts.join('/');
    if (!parts.length || parts.some(part => part === '.' || part === '..') || Object.hasOwn(files, path)) {
        element('new-file-error').textContent = '겹치지 않는 파일 이름을 입력해 주세요. 경로에 . 또는 ..는 쓸 수 없습니다.';
        return;
    }
    persist();
    files[path] = '';
